...
function QuickStats({ unit }: { unit: string }) {
  ...
  const todayEntries = history.filter(
    (e) => e.timestamp.startsWith(todayDate) && e.unit === unit,
  );

  const count = todayEntries.length;
  const avg =
    count > 0
      ? (todayEntries.reduce((s, e) => s + e.value, 0) / count).toFixed(1)
      : null;
  const inRange = todayEntries.filter(
    (e) =>
      getColorClass(
        e.value,
        e.unit as Unit,
  ...
  const highs = todayEntries.filter(
    (e) =>
      getColorClass(
        e.value,
        e.unit as Unit,
  ...
  const lows = todayEntries.filter(
    (e) =>
      getColorClass(
        e.value,
        e.unit as Unit,
  ...
  return (
    ...
    >
      ...
      <View style={styles.statsRow}>
        <View style={styles.statItem}>
          <Text
            style={[
              styles.statNumber,
              { color: inRange > 0 ? colors.normal : colors.text },
            ]}
          >
            {inRange}
          </Text>
        ...
        >
          <Text
            style={[
              styles.statNumber,
              { color: highs > 0 ? colors.high : colors.text },
            ]}
          >
            {highs}
          </Text>
        ...
        >
          <Text
            style={[
              styles.statNumber,
              { color: lows > 0 ? colors.low : colors.text },
            ]}
          >
            {lows}
          </Text>
      ...
      {count === 0 && (
        <Text style={[styles.statsEmpty, { color: colors.textFaint }]}>
          {t.noReadingsToday}
        </Text>
      )}
    </View>
          ...
          <ScrollView contentContainerStyle={styles.popupScroll}>
            ...
            >
              <Text style={[styles.popupValueLine, { color: colors.text }]}>
                {t.hypoValueLine}{" "}
                <Text style={[styles.popupValueRed, { color: colors.red }]}>
                  {glucoseValue} {unit}
                </Text>
              </Text>
              <Text style={[styles.popupSection, { color: colors.text }]}>
                {t.hypoSection1}
              </Text>
              {[t.hypoTip1, t.hypoTip2, t.hypoTip3, t.hypoTip4, t.hypoTip5].map(
                (tip, i) => (
                  <Text
                    key={i}
                    style={[styles.popupTip, { color: colors.textMuted }]}
                  >
                    {tip}
                  </Text>
                ),
              ...
              <PressBtn
                ...
                onPress={onClose}
              >
                <Text style={[styles.closeBtnText, { color: colors.red }]}>
                  {t.close}
                </Text>
              </PressBtn>
            </View>
        ...
        <View style={{ width: "100%", maxHeight: "80%" }}>
          <ScrollView contentContainerStyle={styles.popupScroll}>
            ...
            >
              <Text style={[styles.popupValueLine, { color: colors.text }]}>
                {t.hyperValueLine}{" "}
                <Text style={{ fontWeight: "bold", color: colors.high }}>
                  {glucoseValue} {unit}
                </Text>
              </Text>
              ...
              {correctionDose !== null && correctionDose > 0 && (
                <>
                  ...
                  <Text
                    style={[styles.popupCorrectionDose, { color: colors.high }]}
                  >
                    {correctionDose} {t.hyperUnits}
                  </Text>
                  <Text style={[styles.popupTip, { color: colors.textMuted }]}>
                    {t.hyperEstimateOnly}
                  </Text>
              ...
              {[t.hyperTip1, t.hyperTip2, t.hyperTip3, t.hyperTip4].map(
                (tip, i) => (
                  <Text
                    key={i}
                    style={[styles.popupTip, { color: colors.textMuted }]}
              ...
              <PressBtn
                style={[
                  styles.closeBtn,
                  { borderColor: colors.high, backgroundColor: "transparent" },
                ]}
                onPress={onClose}
              >
                <Text style={[styles.closeBtnText, { color: colors.high }]}>
                  {t.close}
                </Text>
              </PressBtn>
            </View>
...
export default function HomeScreen() {
  ...
  const [guidedFlowType, setGuidedFlowType] = useState<"hypo" | "hyper">(
    "hypo",
  );
  ...
  const handleBleReading = (reading: GlucoseReading) => {
    ...
    setSettings({ glucoseUnit: reading.unit });
  ...
  const liveClass =
    inputValue && !isNaN(liveNum) && liveNum > 0
      ? getColorClass(liveNum, unit, settings.glucoseLow, settings.glucoseHigh)
      : null;
  ...
  return (
    <View style={[styles.safeArea, { backgroundColor: colors.bg }]}>
      ...
      >
        ...
        >
          ...
          <Text style={[styles.appDescription, { color: colors.textMuted }]}>
            {t.appDescription}
            <Text style={[styles.highlight, { color: colors.red }]}>
              {t.appDescriptionHighlight}
            </Text>
          </Text>
          ...
          >
            ...
            <Text style={[styles.bleBtnText, { color: colors.red }]}>
              {t.connectGlucometer}
            </Text>
          </TouchableOpacity>

          <TouchableOpacity
            onPress={() => setShowCompatibleDevices(true)}
            activeOpacity={0.7}
            style={styles.compatLink}
          >
            <Text style={[styles.compatLinkText, { color: colors.textMuted }]}>
              {t.compatibleDevices}
            </Text>
          </TouchableOpacity>
          ...
          <View
            style={[
              styles.divider,
              { backgroundColor: isDark ? "#7a1c1a" : "#f2c0c0" },
            ]}
          />
              ...
              onPress={() => {
                if (unit !== "mg/dL") {
                  ...
                  if (Number.isFinite(v) && v > 0)
                    setInputValue(String(Math.round(v * 18.0182)));
                }
                setUnit("mg/dL");
                setSettings({ glucoseUnit: "mg/dL" });
              }}
              activeOpacity={0.8}
            >
              <Text
                style={[
                  styles.unitLabel,
                  { color: unit === "mg/dL" ? colors.red : colors.textMuted },
                ]}
              >
                mg/dL
              </Text>
            ...
            >
              <View
                style={[
                  ...
                  unit === "mmol/L" && styles.sliderKnobRight,
                ]}
              />
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => {
                if (unit !== "mmol/L") {
                  const v = parseFloat(inputValue);
                  if (Number.isFinite(v) && v > 0)
                    setInputValue((v / 18.0182).toFixed(1));
                }
                setUnit("mmol/L");
                setSettings({ glucoseUnit: "mmol/L" });
              }}
              activeOpacity={0.8}
            >
              <Text
                style={[
                  styles.unitLabel,
                  { color: unit === "mmol/L" ? colors.red : colors.textMuted },
                ]}
              >
                mmol/L
              </Text>
          ...
          <View style={styles.fastingGrid}>
            {FASTING_OPTIONS.map((opt) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  setFasting(fasting === opt.value ? "" : opt.value)
                }
                activeOpacity={0.75}
              >
                <View
                  style={[
                    ...
                    {
                      borderColor:
                        fasting === opt.value ? colors.red : colors.border,
                      backgroundColor:
                        fasting === opt.value ? colors.red : colors.inputBg,
                    },
                  ]}
                />
                <Text style={[styles.fastingText, { color: colors.text }]}>
                  {opt.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          {glucoseValue !== null && (
            ...
            >
              ...
              <Text
                style={[styles.resultInterpretation, { color: resultColor }]}
              >
                {statusIcon(getInterpretation(glucoseValue, unit))}{" "}
                {interpretationLabel(getInterpretation(glucoseValue, unit))}
              </Text>
              {cls !== "normal" && (
                <PressBtn
                  style={[
                  ...
                  activeOpacity={0.75}
                >
                  <Text style={[styles.seeHelpBtnText, { color: colors.red }]}>
                    {t.seeHelp}
                  </Text>
                </PressBtn>
              )}
          ...
          >
            <Text style={[styles.caregiverLinkText, { color: colors.red }]}>
              {t.useAppAsCaregiver}
            </Text>
          </TouchableOpacity>
          ...
          >
            <Text style={[styles.tipTitle, { color: colors.textMuted }]}>
              {t.quickTip}
            </Text>
          ...
          <PressBtn
            style={[
              styles.howToBtn,
              { borderColor: colors.red, backgroundColor: colors.bgCard },
            ]}
            onPress={() => setHasSeenOnboarding(false)}
            activeOpacity={0.75}
          >
            <Text style={[styles.howToBtnText, { color: colors.red }]}>
              {t.howToUseApp}
            </Text>
          </PressBtn>
        </ScrollView>
      ...
      <HypoPopup
        ...
        unit={unit}
        onClose={() => setShowHypoPopup(false)}
      />
      <HyperPopup
        visible={showHyperPopup}
        glucoseValue={glucoseValue!}
        unit={unit}
        onClose={() => setShowHyperPopup(false)}
      />
        ...
        <View style={[styles.overlayBg, { backgroundColor: colors.overlay }]}>
          <View style={[styles.compatSheet, { backgroundColor: colors.bg }]}>
            ...
            {[
              ...
              >
                <View
                  style={[styles.compatDot, { backgroundColor: colors.normal }]}
                />
                <View>
                  <Text style={[styles.compatBrand, { color: colors.text }]}>
                    {brand}
                  </Text>
                  <Text
                    style={[styles.compatModels, { color: colors.textMuted }]}
                  >
                    {models}
                  </Text>
                </View>
      ...
      >
        <View style={[styles.overlayBg, { backgroundColor: colors.overlay }]}>
          <View
            style={[
              styles.caregiverSheet,
              { backgroundColor: colors.bg, borderColor: colors.border },
            ]}
          >
            <Text style={[styles.caregiverTitle, { color: colors.text }]}>
              {t.caregiverModeTitle}
            </Text>
            <Text
              style={[
                styles.caregiverInstructions,
                { color: colors.textMuted },
              ]}
            >
              {t.caregiverCodeInstructions}
            </Text>
            <TextInput
              style={[
                styles.caregiverInput,
                {
                  borderColor: caregiverError ? "#e53935" : colors.border,
                  color: colors.text,
                  backgroundColor: colors.inputBg,
                },
              ]}
              ...
              value={caregiverCode}
              onChangeText={(v) => {
                setCaregiverCode(v.replace(/\D/g, "").slice(0, 6));
                setCaregiverError("");
              }}
            ...
            />
            {!!caregiverError && (
              <Text style={styles.caregiverErrorText}>{caregiverError}</Text>
            )}
            <TouchableOpacity
              style={[
                styles.caregiverActivateBtn,
                {
                  backgroundColor:
                    caregiverLoading || caregiverCode.length !== 6
                      ? colors.border
                      : colors.red,
                },
              ]}
            ...
            >
              <Text style={styles.caregiverActivateBtnText}>
                {caregiverLoading ? t.pleaseWait : t.activate}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setShowCaregiverModal(false)}
              activeOpacity={0.7}
              style={styles.caregiverCancelLink}
            >
              <Text
                style={[
                  styles.caregiverCancelText,
                  { color: colors.textMuted },
                ]}
              >
                {t.cancel}
